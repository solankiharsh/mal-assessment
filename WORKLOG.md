...
## 2026-09-18 15:30 +04:00
- Corrected authorization Mermaid diagrams across `README.md`, `ARCHITECTURE_DECISIONS.md`, and `docs/approach.html` to remove non-existent `ERROR` authorization state; invalid/unknown settlement now explicitly modeled as replay error with unchanged auth state.
- Updated CI to a Python matrix (`3.11`, `3.12`) in `.github/workflows/ci.yml` to match the declared runtime support baseline.
- Revised `NUMBERS.md` wording for `requires-python >=3.11` to avoid claiming a language-syntax necessity.
- Added missing concise simplifications to `ARCHITECTURE_DECISIONS.md` (`no HTTP/API/UI surface`, `no overdraft product-limit config`, `no account lifecycle freeze/closure`, `simplified interest product`).
- Removed obsolete `PR_NOTES.md` from the prominent README artifacts list.
- Updated `PR_DESCRIPTION.md` PDF metadata from `3 pages` to `4 pages`.
- Preserved earlier PDF verification entries and appended this final correction pass rather than rewriting prior history.

## 2026-09-18 15:30 +04:00
- Revalidated final PDF after documentation completion updates:
  - current committed artifact: `docs/architecture-tradeoffs.pdf` = `4` pages, `11,052` bytes.
- Preserved prior verification entries as historical checkpoints; this entry records the final artifact metadata for submission.
